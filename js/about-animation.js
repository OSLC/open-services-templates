...
if (Raphael.type === "SVG") {
	...
	$(paper1).data('name','Panel 1: Our goals');
	...
	// Get things moving after a delay
	//addTimeout( paper1start,1000);
	...
	$(paper2).data('name', 'Panel 2: Our process');
	...
	$(paper3).data('name','Panel 3: Our specifications');
	...
	function prepareRestart(paper,restart) {
	
		$(paper).data('watchingForRestart', true);
			
		var restartBG = paper.rect(-5,-5, paper.width+10, paper.height+10)
			.attr({'stroke-width':0, fill:'#000', opacity: 0, 'title': 'Click to watch again'})
			
		var restartIcon = paper.path("M25.882,6.116C23.351,3.577,19.856,2,15.987,2C8.263,2,2.001,8.268,2.001,16 s6.266,14.001,13.988,14.001c5.723,0,10.641-3.44,12.809-8.364l-3.816-1.638c-1.538,3.442-4.984,5.853-8.991,5.853 c-5.427,0-9.843-4.42-9.843-9.852c0-5.432,4.416-9.851,9.843-9.851c2.719,0,5.18,1.022,6.963,2.814l-4.145,4.054h11.176V2 L25.882,6.116z")
			.attr({'stroke-width':.25, 'stroke': '#333', fill: '#FFF', transform: 'S5,5,0,0', opacity : .6, 'title' : 'Click to watch animation again'})
		...
		var restartSet = paper.set([restartBG, restartIcon]);
		
		restartSet.attr('cursor', 'pointer');
				
		restartBG.animate({'opacity':0.1}, 200, '<', function(){
			restartIcon.animate({'transform': '...T300,0'}, '250', '>')
		});
		...
		restartSet.click(function(e){
			...
			if (window._gaq) { // If Analytics is around
				_gaq.push(['_trackEvent', 'About page interaction', 'Restarted animation', $(paper).data('name'), 0]);
			}
	...
	function restartPaper(paper, restart){
		...
		// TO DO: add a check. If there's no restart specified, don't fire it
